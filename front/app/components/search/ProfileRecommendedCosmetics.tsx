...
const ProfileRecommendedCosmetics = () => {
  ...
  useEffect(() => {
    const fetchRecommendedCosmetics = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/search_cosmetics/profile_recommendations`, {
          headers: headers,
          withCredentials: true,
          skin_type: profile.skinType,
          skin_trouble: profile.skinTrouble,
          name: profile.name,
        });
        setRecommendedCosmetics(response.data);
      } catch (error) {
        console.error('レコメンドされたコスメの取得に失敗しました', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (recommendedCosmetics.length === 0) {
    return null;
  }
  ...
  return (
    <div className="flex flex-col items-center">
      <div className='mt-4 w-full max-w-4xl justify-center'>
        <h2 className="text-xl font-semibold mb-4">{profile.skin_type} × {profile.skin_trouble} の<br />{profile.name}さんにはこちらもおすすめ</h2>
...
export default ProfileRecommendedCosmetics;